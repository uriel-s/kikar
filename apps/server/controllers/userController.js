...
const removeFriend = async (req, res, db) => {
    ...
    try {
        ...
        if (userFriendIndex === -1) return res.status(404).json({ message: 'Friend not found' });
        ...
        if (friendIndex === -1) return res.status(404).json({ message: 'User not found in friend\'s list' });
